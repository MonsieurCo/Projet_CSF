...
String datouille = "hello wolrd";

//Define sensor PIN
#define LAPIN A0 // PIN with Light sensor analog output 
#define LPPIN 1 // PIN with Light power input
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
   ...
   /* char tab[donne.length()];
    //for ( int i = 0;i<donne.length(); i++) {
      //tab =+ donne.charAt(i);
      
   //}
   donne.toCharArray(tab, donne.length());*/
    
    LMIC_setTxData2(1, tab, sizeof(tab), 0);
...
void loop() {
  ...
  while(mini.available())
  {
   ...
   if ( c == 'E') 
    ...
    if ( donne.length() >= 73 )
    {
      for(int i = 0; i < datouille.length(); i++)
      {
        tab[i] = (char)datouille.charAt(i);
        Serial.print((char)tab[i]);
      } 
     //Serial.print(donne);
  // char tab[datouille.length()];// = datouille ;
   //datouille.toCharArray(tab, datouille.length()+2);
  Serial.println();
   donne = "";
  counter++;
  os_runloop_once();
